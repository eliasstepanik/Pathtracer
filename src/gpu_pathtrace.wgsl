...
struct Camera { pos: vec3<f32>, _pad0: f32, look_at: vec3<f32>, _pad1: f32, up: vec3<f32>, _pad2: f32, forward: vec3<f32>, _pad3: f32, width: u32, height: u32, fov: f32, sphere_count: u32, plane_count: u32, aperture: f32, focus_dist: f32, _pad4: u32 };
...
struct Sphere { center: vec4<f32>, color: vec4<f32>, radius: f32, metallic: f32, roughness: f32, _pad: f32 };
struct Plane { point: vec4<f32>, normal: vec4<f32>, u: vec4<f32>, v: vec4<f32>, color: vec4<f32>, metallic: f32, roughness: f32, _pad: vec2<f32> };
...
struct Material { color: vec3<f32>, metallic: f32, roughness: f32 };
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var s = 0u; s < params.samples_per_pixel; s = s + 1u) {
        ...
        for (var i = 0u; i < params.max_bounces; i = i + 1u) {
            ...
            if (i >= 2u) { if (rand() > p) { break; } throughput /= p; }
            current_ray.origin = hit.pos + hit_normal * 0.0001;
