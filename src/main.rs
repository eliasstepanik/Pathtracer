...
fn main() {
    ...
    let bar = if !quiet_mode {
        ...
        println!(
            "\nRendering {}x{} image with {} samples... (quiet mode)",
        ...
        .flat_map(|y| {
            ...
            for x in 0..width {
                ...
                let col = renderer::pixel_color(
                    x, y, width, height, samples, aspect, scale, pos, right, real_up, forward,
                    focus, aperture, &objects, &lights, &mut rng,
                );
                row.push(((x, y), col));
            }
            row
